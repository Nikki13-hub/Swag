	...
	function(record, runtime, search) {
		function getInputData() {
			...
			try {
				...
				let s_savesearch_ID = scriptObj.getParameter('custscript_sp_ccje_ss');
				return search.load({
					id: s_savesearch_ID
				});
			} catch (e) {
		...
		function map(context) {
			...
			try {
				...
				let objRec = record.load({
					type: 'crosschargejournal',
					id: context.key
				});
				let lineCount = objRec.getLineCount({
					sublistId: 'audit'
				});
				let period = objRec.getValue('postingperiod');
				let subsidiary = objRec.getValue('subsidiary');
				let sourceIds = [];
				for (let i = 0; i < lineCount; i++) {
					let id = objRec.getSublistValue({
						sublistId: 'audit',
						fieldId: 'relatedrecordurl',
						line: i
					}).split('id=')[1];
					sourceIds.push(id);
				}
				...
				myResults.forEach(function(result) {
					...
					lineData.subsidiary = subsidiary;
		...
		function reduce(context) {
			...
			try {
				...
				objRecord.setValue('custrecord_8qccjed_type', 'Cross Charge');
		...
		function summarize(summary) {
			summary.output.iterator().each((key, value) => {
				...
				log.audit('PROCESSED', 'Cross Charge Journal #' + lookupData.tranid);
			});
			//Grab Map errors
