...
define(['N/search','N/record','N/runtime','N/file','N/email'],function(search,record,runtime,file,email) {
	...
	function getInputData() {
		return search.load('customsearch_8q_item_ful_no_dept_2');
	}
	...
	function reduce(context) {
		...
		let rec = record.load({type: 'itemfulfillment', id: context.key});
	...
	function summarize(summary) {
		...
		if (errors) {
			...
			email.send({
					author: -5,
					recipients: 'obie@8quanta.com',
					attachments: [fileObj],
					subject: 'Inventory Adjustment Errors',
					body: 'errors'
			});
		}
		return true;		
